...
export default function OSDetailScreen() {
  ...
  const navigation =
    useNavigation<NativeStackNavigationProp<RootStackParamList>>(); // <-- DEVE FICAR AQUI NO TOPO!
  ...
  return (
    <View style={styles.container}>
      ...
      <Text>
        <Text style={styles.label}>Cliente:</Text> {os.cliente}
      </Text>
      <Text>
        <Text style={styles.label}>Veículo:</Text> {os.veiculo}
      </Text>
      <Text>
        <Text style={styles.label}>Placa:</Text> {os.placa}
      </Text>
      <Text>
        <Text style={styles.label}>Descrição:</Text> {os.descricao}
      </Text>
      <Text>
        <Text style={styles.label}>Status:</Text> {os.status}
      </Text>
      <Text>
        <Text style={styles.label}>Criada em:</Text>{" "}
        {new Date(os.dataCriacao).toLocaleString()}
      </Text>
      ...
      {checklist && (
        <View style={{ marginTop: 30 }}>
          ...
          <Text style={{ marginTop: 10 }}>
            ...
            {new Date(checklist.dataHora).toLocaleString()}
          </Text>
