...
export default function ChecklistEntradaScreen() {
  ...
  const handleSalvarChecklist = async () => {
    if (!assinatura) {
      Alert.alert('Assinatura obrigatória')
      return
    }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} scrollEnabled={scrollHabilitado}>
      ...
      <View style={styles.assinaturaContainer}>
        <Text style={{ fontWeight: 'bold', marginBottom: 10 }}>Assinatura do Cliente:</Text>
        ...
        <View style={styles.botoesAssinatura}>
          <Button title="Limpar" onPress={() => signatureRef.current?.clearSignature()} />
